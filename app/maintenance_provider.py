...
from .maintenance_models import MaintenanceBackend, ProviderType
...
_PROVIDER_CAPABILITY_MATRIX = {
    ProviderType.NATIVE_PODMAN: ProviderCapabilities(
        host_mutation=True,
        workload_mutation=True,
        cluster_settings=True,
        lifecycle_api=True,
        observation=True,
        recovery=True,
    ),
    ProviderType.ADOPTED_PODMAN: ProviderCapabilities(
        workload_mutation=True,
        cluster_settings=True,
        lifecycle_api=True,
        observation=True,
    ...
    ProviderType.EXTERNAL_API: ProviderCapabilities(
        cluster_settings=True,
        lifecycle_api=True,
        observation=True,
        recovery=True,
...
def capability_matrix(provider_type: ProviderType | str) -> dict[str, bool]:
    ...
    return _PROVIDER_CAPABILITY_MATRIX[provider].model_dump()
...
class ProviderProfile(BaseModel):
    ...
    def capabilities(self) -> ProviderCapabilities:
        ...
        effective = {
            name: maximum[name] and requested.get(name, maximum[name])
            for name in maximum
        }
        if self.ownership_state is not OwnershipState.VERIFIED:
            effective = {
                name: value if name == ProviderCapability.OBSERVATION.value else False
...
def require_capability(profile: ProviderProfile, capability: ProviderCapability | str) -> None:
    ...
    if not getattr(profile.capabilities, selected.value):
        raise PermissionError(
            f"Provider {profile.provider_type.value} in {profile.ownership_state.value} state "
            f"does not allow {selected.value}"
        )

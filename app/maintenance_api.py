...
from app.maintenance_lifecycle import TransitionError
from app.maintenance_models import MaintenancePolicy
from app.maintenance_observation import collect_host_reboot_planning_data
from app.maintenance_planning import canonical_hash
...
def require_cluster(connection, cluster_id: int):
    if not connection.execute("SELECT 1 FROM clusters WHERE id=?", (cluster_id,)).fetchone():
        raise HTTPException(404, "Cluster not found")


def capability_revision() -> str:
    return canonical_hash({"schema": 1, "capabilities": core.MAINTENANCE_CAPABILITIES})
...
def maintenance_plan_response(repository: MaintenanceRepository, record):
    capabilities = MaintenanceActionCapabilities(
        pause=core.MAINTENANCE_CAPABILITIES["host_reboot"],
        resume=core.MAINTENANCE_CAPABILITIES["host_reboot"],
        cancel=core.MAINTENANCE_CAPABILITIES["host_reboot"],
        recover=core.MAINTENANCE_CAPABILITIES["host_reboot"],
    )
    return {
...
async def get_maintenance_policy(cluster_id: int, _: Annotated[str, Depends(core.user)]):
    with core.db() as connection:
        require_cluster(connection, cluster_id)
...
async def put_maintenance_policy(
    ...
    username: Annotated[str, Depends(core.user)],
):
...
async def create_host_maintenance_plan(
    ...
    if not core.MAINTENANCE_CAPABILITIES["planning"]:
    ...
    try:
        ...
        with core.db() as connection:
            ...
            data = collect_host_reboot_planning_data(
                ...
                node_shutdown_backend_enabled=core.MAINTENANCE_CAPABILITIES["node_shutdown_backend"],
            )
...
async def get_maintenance_plan(plan_id: str, _: Annotated[str, Depends(core.user)]):
    try:
        with core.db() as connection:
            repository = MaintenanceRepository(connection)
...
async def maintenance_action(
    ...
    if not core.MAINTENANCE_CAPABILITIES["host_reboot"]:

...
from app.maintenance_service import HostRebootPlanningData
from app.maintenance_provider import provider_profile_from_record
...
def collect_host_reboot_planning_data(
    ...
    *,
    ...
    target = connection.execute("SELECT * FROM nodes WHERE id=?", (node_id,)).fetchone()
    if not target:
        raise KeyError(node_id)

    target_assignments = connection.execute(
        "SELECT * FROM cluster_assignments WHERE node_id=? AND state='active' ORDER BY id", (node_id,),
    ).fetchall()

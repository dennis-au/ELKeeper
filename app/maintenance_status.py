...
from .maintenance_lifecycle import (
...
from .maintenance_store import CheckpointRecord, HostStateRecord, PlanRecord, StepRecord


_STEP_STATES = {
    MaintenanceStepState.PENDING: "pending",
    MaintenanceStepState.EXECUTING: "active",
    MaintenanceStepState.VERIFIED: "completed",
    MaintenanceStepState.SKIPPED: "skipped",
    MaintenanceStepState.FAILED: "failed",
    MaintenanceStepState.RECOVERY_REQUIRED: "recovery_required",
}

_SAFE_CONTROL_CHECKPOINTS = frozenset({
...
def _host_boot(checkpoints: Sequence[CheckpointRecord], evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if "reboot.return-discovered" in keys:
        ...
        state = "returned"
        verified = bool(evidence.get("boot_transition_verified"))
        ...
        verified = False
    return {
        "state": evidence.get("state", state),
        "bootTransitionVerified": bool(evidence.get("boot_transition_verified", verified)),
        "observedAt": evidence.get("observed_at"),
...
def _executor(checkpoints: Sequence[CheckpointRecord], evidence: Mapping[str, Any]) -> dict[str, Any]:
    keys = {item.checkpoint_key for item in checkpoints}
    state = "not_staged"
    if "reboot.executor-staged" in keys:
        state = "staged"
    ...
    state = str(evidence.get("state", state))
    return {
        "state": state,
        "signatureVerified": evidence.get("manifest_signature_verified"),
        "resultIdentityVerified": evidence.get("result_identity_verified"),
        "resultImported": evidence.get("result_imported"),
        "reason": evidence.get("reason"),
        "observedAt": evidence.get("observed_at"),
        "checks": list(evidence.get("checks", ())),
    }
...
def _actions(
    ...
    } and capabilities.cancel:
        enabled = at_cancel_boundary and not unresolved
        controls["cancel"] = {
            "enabled": enabled,
            "reason": None if enabled else "Cancellation is blocked until active effects and temporary cluster settings are resolved.",
        }
    if state == MaintenanceState.RECOVERY_REQUIRED and capabilities.recover:
        controls["recover"] = {
            "enabled": True,
            "requiresSafeCheckpoint": False,
            "reason": "Recovery begins with state rediscovery and does not assume the last command completed.",
        }
    return controls
...
def serialize_maintenance_operation(
    ...
    *,
    ...
    active_step = next(
        (item for item in ordered_steps if item.state in {MaintenanceStepState.EXECUTING, MaintenanceStepState.RECOVERY_REQUIRED}),
        None,
    )
    last_verified = next(
        (item for item in reversed(ordered_steps) if item.state == MaintenanceStepState.VERIFIED),
        None,
    )
    ...
    if latest is not None:
        checkpoint_state = (
            ...
            else "verified" if latest.side_effect_state == SideEffectState.VERIFIED
            else "active" if latest.side_effect_state == SideEffectState.MAY_HAVE_STARTED
            else "pending"
        )
        active_checkpoint = {
            "id": latest.checkpoint_key,
            "label": _checkpoint_label(latest.checkpoint_key),
            "state": checkpoint_state,
            "safeForOperatorAction": safe,
            "detail": safe_reason,
            "updatedAt": latest.classified_at or latest.created_at,
        }
    elif active_step is not None:
        active_checkpoint = {
            "id": active_step.step_key,
            "label": _checkpoint_label(active_step.step_kind),
            "state": "recovery_required" if active_step.state == MaintenanceStepState.RECOVERY_REQUIRED else "active",
            "safeForOperatorAction": safe,
            "detail": safe_reason,
            "updatedAt": active_step.updated_at,
        }

    progress = {
        "lifecycleState": plan.lifecycle_state.value,
        "progress": {"completed": completed, "total": len(ordered_steps)},
        "activeCheckpoint": active_checkpoint,
        "lastVerifiedCheckpoint": (
            {"label": _checkpoint_label(last_verified.step_kind), "verifiedAt": last_verified.finished_at}
            if last_verified else None
        ),
        "hostBoot": _host_boot(ordered_checkpoints, evidence.get("host_boot", {})),
        "cleanup": cleanup,
        "executor": _executor(ordered_checkpoints, evidence.get("executor", {})),
    }
    ...
    return redact_structure({
        ...
        "safe_checkpoint_reason": safe_reason,
        "action_controls": _actions(
            plan.lifecycle_state,
            capabilities or MaintenanceActionCapabilities(),
            safe,
            latest,
            cleanup,
        ),

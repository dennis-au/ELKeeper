...
PLAN_TRANSITIONS: dict[MaintenanceState, frozenset[MaintenanceState]] = {
    MaintenanceState.DRAFT: frozenset({
        MaintenanceState.READY,
        MaintenanceState.BLOCKED,
        MaintenanceState.CANCELLED,
    }),
    MaintenanceState.READY: frozenset({
        MaintenanceState.EXECUTING,
        MaintenanceState.BLOCKED,
        MaintenanceState.CANCELLED,
    }),
    MaintenanceState.BLOCKED: frozenset({
    ...
    MaintenanceState.PAUSED: frozenset({
        MaintenanceState.EXECUTING,
        MaintenanceState.RECOVERY_REQUIRED,
        MaintenanceState.FAILED,
        MaintenanceState.CANCELLED,
    }),
    MaintenanceState.RECOVERY_REQUIRED: frozenset({
        MaintenanceState.EXECUTING,
        MaintenanceState.SUCCEEDED,
        MaintenanceState.FAILED,
        MaintenanceState.CANCELLED,
...
STEP_TRANSITIONS: dict[MaintenanceStepState, frozenset[MaintenanceStepState]] = {
    MaintenanceStepState.PENDING: frozenset({
        MaintenanceStepState.EXECUTING,
        MaintenanceStepState.SKIPPED,
        MaintenanceStepState.FAILED,
    }),
    MaintenanceStepState.EXECUTING: frozenset({
        MaintenanceStepState.VERIFIED,
        MaintenanceStepState.FAILED,
        MaintenanceStepState.RECOVERY_REQUIRED,
    }),
    MaintenanceStepState.RECOVERY_REQUIRED: frozenset({
        MaintenanceStepState.EXECUTING,
        MaintenanceStepState.VERIFIED,
        MaintenanceStepState.FAILED,
        MaintenanceStepState.SKIPPED,
...
HOST_TRANSITIONS: dict[HostMaintenanceState, frozenset[HostMaintenanceState]] = {
    ...
    HostMaintenanceState.PLANNING: frozenset({
        HostMaintenanceState.AVAILABLE,
        HostMaintenanceState.MAINTENANCE,
        HostMaintenanceState.RECOVERY_REQUIRED,
    }),
    HostMaintenanceState.MAINTENANCE: frozenset({
        HostMaintenanceState.AVAILABLE,
        HostMaintenanceState.DRAINING,
        HostMaintenanceState.RECOVERY_REQUIRED,
    }),
    HostMaintenanceState.DRAINING: frozenset({
        HostMaintenanceState.AVAILABLE,
        HostMaintenanceState.MAINTENANCE,
        HostMaintenanceState.RECOVERY_REQUIRED,
    }),
    HostMaintenanceState.RECOVERY_REQUIRED: frozenset({
        HostMaintenanceState.AVAILABLE,
...
def redact_structure(value: Any) -> Any:
    if isinstance(value, Mapping):
        return {
            str(key): "[REDACTED]" if _sensitive_key(key) else redact_structure(item)

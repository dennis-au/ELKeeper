"""Maintenance plan compilation and execution-validation contract."""
...
def canonical_json(value: Any) -> str:
    return json.dumps(_canonical_value(value), ensure_ascii=True, separators=(",", ":"), sort_keys=True, allow_nan=False)
...
def _plan_payload(plan: CompiledPlan | dict) -> dict:
    payload = plan.model_dump(mode="python") if isinstance(plan, CompiledPlan) else dict(plan)
    payload.pop("plan_hash", None)
    return payload


def compile_plan(
    *, target: PlanningTarget, policy: MaintenancePolicy, policy_revision: int,
    backend: MaintenanceBackend, observation: ObservationSnapshot,
    predicates: tuple[PredicateResult, ...], impact: ImpactManifest,
    steps: tuple[PlanStep, ...], rollback_boundaries: tuple[RollbackBoundary, ...],
    created_at: datetime, idempotency_key: str | None = None,
) -> CompiledPlan:
    ...
    payload = {
        "schema_version": 1, "idempotency_key": idempotency_key, "target": target,
        "policy": policy, "policy_revision": policy_revision, "backend": backend,
        "observation": observation, "predicates": predicates, "impact": impact,
        "steps": ordered_steps,
...
def validate_plan_for_execution(
    plan: CompiledPlan, *, now: datetime, expected_plan_hash: str,
    current_policy_revision: int, current_capability_revision: str,

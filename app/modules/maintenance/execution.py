...
from .lifecycle import LockScope, MaintenanceState
from .store import (
    LockRequest,
...
from app.modules.platform import RunDescriptor, start_run_in_connection
...
class MaintenanceExecutionService:
    ...
    def fail(self, ticket: MaintenanceActionTicket, *, error_category: str) -> PlanRecord:
        ...
        MaintenanceReadRepository.from_connection(self.repository.connection).mark_run_status(
            ticket.run_id,
            "recovery_required",
            finished_at=iso_timestamp(now),
            log_suffix=f"Maintenance adapter stopped at a recovery boundary ({self._safe_error_category(error_category)}).\n",
        )
        self.repository.record_audit(
            username=ticket.request.requested_by,
    ...
    def _validate_current_revisions(self, plan: PlanRecord) -> None:
        ...
        workloads = WorkloadRepository.from_connection(self.repository.connection)
        hosts = HostRepository.from_connection(self.repository.connection)
        for item in manifest.get("assignment_revisions", ()):
            ...
            revision = workloads.revision_in_connection(self.repository.connection, assignment_id)
        ...
        if plan.target_node_id is not None:
            if not hosts.is_enabled_in_connection(self.repository.connection, plan.target_node_id):
    ...
    def _attach_run(self, plan: PlanRecord, action: MaintenanceAction) -> int:
        run_repository = MaintenanceReadRepository.from_connection(self.repository.connection)
        if plan.run_id is not None:
            run = run_repository.run(plan.run_id)
            if run is None:
            ...
            run_repository.mark_run_running(plan.run_id)
            return plan.run_id
        target = f"plan:{plan.id}"
        if plan.target_node_id is not None:
            host = HostRepository.from_connection(self.repository.connection).get(plan.target_node_id)
            if host:
                target = str(host["name"])
        descriptor = RunDescriptor(
            kind=f"maintenance-{plan.operation_kind}",
            target=target,
            context={
        ...
        run = start_run_in_connection(self.repository.connection, descriptor)
        run_id = run.run_id
        self.repository.attach_run_id(plan.id, run_id)
        return run_id

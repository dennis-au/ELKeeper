...
from .maintenance_recovery import RecoveryDecision, RecoveryEvidence, classify_recovery
...
def _add_observation_identity_columns(connection: sqlite3.Connection) -> None:
    if not connection.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='host_runtime_observations'"
    ).fetchone():
        return
    ...
    try:
        ...
        applied_rows = {
            ...
            for row in connection.execute(
                "SELECT version,name,checksum FROM maintenance_schema_migrations ORDER BY version"
            ).fetchall()
        }
        ...
        for version, name, checksum, apply_migration in MAINTENANCE_MIGRATIONS:
            ...
            connection.execute(
                "INSERT INTO maintenance_schema_migrations(version,name,checksum,applied_at) VALUES(?,?,?,?)",
                (version, name, checksum, iso_timestamp()),
            )
...
class MaintenanceRepository:
    ...
    def prepare_startup_recovery(self) -> StartupRecovery:
        rows = self.connection.execute("""
            SELECT maintenance_plans.*,runs.status AS linked_run_status
            FROM maintenance_plans
            LEFT JOIN runs ON runs.id=maintenance_plans.run_id
            WHERE maintenance_plans.lifecycle_state IN ('executing','paused','recovery_required')
               OR runs.status IN ('queued','running','recovery_required')
            ORDER BY maintenance_plans.created_at,maintenance_plans.id
        """).fetchall()
        ...
        if protected_run_ids:
            placeholders = ",".join("?" for _ in protected_run_ids)
            self.connection.execute(
                f"UPDATE runs SET status='recovery_required',finished_at=COALESCE(finished_at,CURRENT_TIMESTAMP),"
                "log=log || ? WHERE id IN (" + placeholders + ") AND status IN ('queued','running')",
                ("Controller restarted during a maintenance operation; state rediscovery is required.\n", *sorted(protected_run_ids)),
            )
    ...
    def observe_conflicts(
        ...
        *,
        ...
        node = self.connection.execute("SELECT id,name FROM nodes WHERE id=?", (node_id,)).fetchone()
        if not node:
            raise RecordNotFound(f"Node {node_id} was not found")
        membership_clusters = {
            row["cluster_id"]
            for row in self.connection.execute("SELECT cluster_id FROM memberships WHERE node_id=?", (node_id,))
        }
        host_assignments = self.connection.execute(
        ...
        if cluster_ids:
            ...
            cluster_assignments = self.connection.execute(
        ...
        active_plan_rows = self.connection.execute("""
        ...
        lock_rows = self.connection.execute("""
        ...
        active_run_rows = self.connection.execute("""
        ...
        batch_rows = self.connection.execute("""
            ...
            FROM workload_change_batches
            JOIN runs ON runs.id=workload_change_batches.run_id
            WHERE runs.status IN ('queued','running','recovery_required')
        """).fetchall()
        batch_run_ids = set()
        for row in batch_rows:
            if row["cluster_id"] not in cluster_ids or row["run_id"] in excluded_run_ids:
                continue
            batch_run_ids.add(row["run_id"])
            identifiers.add(f"workload-batch:{row['run_id']}")

        scoped_names = {node["name"], *cluster_names}
        linked_run_ids |= assignment_run_ids | batch_run_ids
        for row in active_run_rows:
            ...
            context_node_ids = context_ids("node_id", "node_ids")
            context_cluster_ids = context_ids("cluster_id", "cluster_ids")
            context_assignment_ids = context_ids("assignment_id", "assignment_ids")
            ...
            context_matches = (
                node_id in context_node_ids
                or bool(cluster_ids.intersection(context_cluster_ids))
                or bool(cluster_assignment_ids.intersection(context_assignment_ids))
            )
    ...
    def record_audit(
        ...
        *,
        ...
        cursor = self.connection.execute(
            ...
            (username, action, cluster_id, item_id, _json(detail)),
        )

...
function HostEditor({ node, cluster, onClose, onRun }: { node?: NodeRecord; cluster?: Cluster; onClose: () => void; onRun: (runId: number) => void }) {
  ...
  const testPassword = async () => {
    ...
    try {
      const result = await hostApi.testPassword({
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      if (node) await hostApi.save(node.id, { name: form.name, address: form.address, ssh_user: form.ssh_user, ssh_port: form.ssh_port, enabled: form.enabled, ssh_host_key: form.ssh_host_key });
      else {
        const result = await hostApi.save<{ run_id: number }>(undefined, {
...
export function HostsWorkspace() {
  ...
  const { data: dashboard } = useQuery({ queryKey: ['dashboard'], queryFn: dashboardApi.snapshot, refetchInterval: 10000 });
  ...
  const maintenancePlanQuery = useQuery<HostMaintenancePlanResponse>({
    ...
    queryFn: () => hostApi.getMaintenancePlan<HostMaintenancePlanResponse>(maintenancePlanId!),
  ...
  const runAction = async () => {
    ...
    try {
      if (confirm.action === 'removeLegacyKnownHosts') {
        await hostApi.removeLegacyKnownHosts(confirm.node.id);
      }
        ...
        const result = await hostApi.remove(confirm.node.id, query.toString());
        if (result?.run_id) watchRun(result.run_id);
      }
      else {
        const result = await hostApi.action(confirm.node.id, confirm.action);
  ...
  const probe = async (node: NodeRecord) => {
    try { const result = await hostApi.probe(node.id); watchRun(result.run_id); }
  ...
  const installKey = async () => {
    ...
    try {
      const result = await hostApi.installControllerKey(keyInstall.id, bootstrapPassword);
  ...
  const saveZone = async () => {
    ...
    try {
      const result = await hostApi.updateZone(zoneEdit.id, { cluster_id: selectedCluster.id, zone_id: zoneId });
  ...
  const createMaintenancePlan = async () => {
    ...
    try {
      const result = await hostApi.createMaintenancePlan<HostMaintenancePlanResponse>(maintenanceNode.id, {
        operation: 'reboot',
  ...
  const runMaintenanceAction = async (action: MaintenanceAction) => {
    ...
    try {
      const result = await hostApi.maintenanceAction(maintenancePlanId, action);

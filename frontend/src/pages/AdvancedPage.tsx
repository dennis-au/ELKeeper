...
export function AdvancedPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => api<{ items: SensitiveItem[] }>(`/api/clusters/${selectedCluster!.id}/sensitive-items`),
  });
  ...
  const reveal = async (request: RevealState, token: string) => {
    ...
    const result = await api<{ value: string; hide_after: number }>(`/api/clusters/${selectedCluster.id}/sensitive-items/${encodeURIComponent(request.item.id)}/reveal`, { method: 'POST', ...jsonBody({ grant_token: token, purpose: request.purpose }), suppressAuthExpiry: true });
  ...
  const reauthenticate = async () => {
    ...
    setError('');
    try {
      const response = await api<{ grant_token: string; expires_in: number }>('/api/auth/reveal-grants', { method: 'POST', ...jsonBody({ cluster_id: selectedCluster.id, password }), suppressAuthExpiry: true });

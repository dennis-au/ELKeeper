  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      const result = await api<{ id?: number }>(cluster ? `/api/clusters/${cluster.id}` : '/api/clusters', {
        ...
        ...jsonBody({ name, theme_color: themeColor, ...(cluster ? { desired_version: cluster.desired_version } : {}), ports: legacyPortsFromRolePorts(rolePorts), role_ports: rolePorts, network_defaults: { mode: networkMode }, zoning: { mode: zoningMode, zones: normalizedZones }, elasticsearch_settings: cluster?.elasticsearch_settings || defaultSettings }),
      });
...
function ZoningPanel({ cluster }: { cluster: Cluster }) {
  ...
  const mutation = useMutation({
    mutationFn: () => api<{ run_id: number }>(`/api/clusters/${cluster.id}/zoning/apply`, { method: 'POST' }),

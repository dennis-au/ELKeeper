...
function StoragePathPicker({ nodeId, clusterSlug, role, value, onChange, autoFocus = false }: {
  ...
  const mountsQuery = useQuery({
    ...
    retry: false,
    staleTime: 15_000,
    queryFn: () => api<StorageMountResponse>(`/api/nodes/${nodeId}/storage`),
  });
  ...
  const submit = async () => {
    ...
    try {
      if (state.type === 'resources') {
        ...
        await api(`/api/clusters/${network.cluster_id}/members/${network.node_id}`, { method: 'PUT', ...jsonBody({ node_id: network.node_id, network_mode: network.network_mode, data_interface: network.data_interface, data_address: network.data_address, user_interface: network.user_interface, user_address: network.user_address }) });
        await completed();
      } else if (state.type === 'detach') {
        stageChange('detach', state.assignment);
      } else if (state.type === 'purge') {
        const result = await api<{ run_id?: number }>(`/api/assignments/${state.assignment.id}?mode=${state.type}`, { method: 'DELETE' });
...
export function RolesPage() {
  ...
  const addMember = async (event: React.FormEvent) => {
    ...
    try {
      await api(`/api/clusters/${selectedCluster.id}/members`, { method: 'POST', ...jsonBody({ ...member, node_id: Number(member.node_id), data_interface: member.network_mode === 'shared' ? member.user_interface : member.data_interface, data_address: member.network_mode === 'shared' ? member.user_address : member.data_address }) });
  ...
  const removeMember = async (item: Membership) => {
    try { await api(`/api/clusters/${item.cluster_id}/members/${item.node_id}`, { method: 'DELETE' }); await refresh(); }
    catch (reason) { setError((reason as Error).message); }
  };
  ...
  return <div className="page-stack">
    ...
    <section className="section-band">
      ...
      <form className="form-grid role-form" onSubmit={addMember}>
        ...
        <EuiFormRow label="Traffic mode"><EuiSelect value={member.network_mode} onChange={(event) => setMember({ ...member, network_mode: event.target.value })} options={[{ value: 'shared', text: 'Shared NIC' }, { value: 'dedicated', text: 'Dedicated NICs' }]} /></EuiFormRow>

...
export function ActionConsole({ collapseKey, watchedRunId, onWatch }: Props) {
  ...
  const { data: runs = [] } = useQuery({ queryKey: ['runs'], queryFn: queries.runs, refetchInterval: 3000 });
  ...
  useEffect(() => {
    ...
    const events = new EventSource(`/api/runs/${selected.id}/events?token=${encodeURIComponent(selected.events_token)}`);
